...
| Formal Oracle structural + ledger + media + frame + private run-store + composition/request-envelope/prompt-renderer slices | PASS only as non-executable composed infrastructure | externally pinned composition 已绑定 generation-0 HEAD、`SEALED_READY` 与全 `PENDING`。strict canonical envelope 闭合 plan/spec/verified visual bytes；deterministic prompt renderer 从 verified selected transcript 与固定 template/schema 重建，同 case 四臂 task/transcript 一致且不暴露私有 arm/case/Gold 标识。Store 只收 branded artifact并在 durable reload 时重解析；provider wire/account/endpoint、model tokenizer、authoritative rights/withdrawal head、不可变 toolchain capsule、composition 外部签名/WORM、不可回滚 external HEAD、盲评统计仍未完成，因此 `api_execution_allowed=false` |
...
3. 下一步实现只接收 branded envelope 的 prepared transport adapter，证明 captured provider body 与 actual wire 等价并绑定 provider account/endpoint；另以 model-specific tokenizer/image accounting 关闭 input budget 门。再接 authoritative rights/withdrawal head、不可变 toolchain capsule、composition 外部签名/WORM、external monotonic HEAD、私有答案键、公共盲评包与统计器。teacher-only 违规和无证据学生结果必须原样保留为盲评失败样本，不能由运行存储选择性过滤。全部通过后才可能由独立执行 gate 解除 `api_execution_allowed=false`；所有结果继续保持 `TBD`；
