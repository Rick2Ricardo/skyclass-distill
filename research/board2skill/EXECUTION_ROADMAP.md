...
> 2026-08-12 Formal Oracle composition 第一薄片补充：已按 `registry → Gold ledger → media/ASR/frame → run` 固定锁序，把 externally pinned Ed25519 registry capability、当前 Signed Gold snapshot、formal manifest/spec/resource root、结构 schedule、媒体/签字 ASR、source-frame proofs、run/execution-plan 与 create-once generation-0 HEAD 原子组合。所有 caller JSON/bytes 与 trusted key maps 先做独立快照；run 锁内要求 exact external pin、`SEALED_READY`、全请求 `PENDING` 且无 provenance。成功只在 callback 内借出 `composition_attested_only` WeakSet capability；持久 record 明示 non-authoritative，rights active head、toolchain capsule、composition 外部签名/WORM、external monotonic HEAD、strict canonical request payload builder、blind package 和 statistics 全部仍 pending，API 保持 false。
...
| Formal Oracle structural + ledger + media + frame + private run-store + composition slices | PASS only as non-executable composed infrastructure | 结构、可信账本、媒体/语音、source-frame 与 run-store 两薄片现已由 externally pinned composition 第一薄片按固定锁序组合，并在 run 锁内绑定 create-once generation-0 external HEAD pin、`SEALED_READY` 与全 `PENDING`。仅 callback-scoped WeakSet capability 具有临时可信含义；自哈希 record 明示 non-authoritative。strict canonical request payload builder、authoritative rights/withdrawal head、不可变 toolchain capsule、composition 外部签名/WORM、不可回滚 external HEAD、盲评统计仍未完成，因此 `api_execution_allowed=false` |
...
3. 下一步实现 strict canonical request payload schema/builder，逐字段证明 transport payload 与 plan 的 model/system/user/visual/seed/temperature/token/timeout/cache/tools 语义；再接 authoritative rights/withdrawal head、不可变 toolchain capsule、composition 外部签名/WORM、external monotonic HEAD、私有答案键、公共盲评包与统计器。teacher-only 违规和无证据学生结果必须原样保留为盲评失败样本，不能由运行存储选择性过滤。全部通过后才可能由独立执行 gate 解除 `api_execution_allowed=false`；所有结果继续保持 `TBD`；
