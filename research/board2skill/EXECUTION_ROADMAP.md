...
| Formal Oracle structural + ledger + media + frame + private run-store + composition/request-envelope/prompt-renderer/Pi-body slices | PASS only as non-executable composed infrastructure | externally pinned composition 已绑定 generation-0 HEAD、`SEALED_READY` 与全 `PENDING`。strict envelope 与 deterministic prompt 从 verified bytes 重建；plan/intent v2 拆分 envelope/body 双 hash，Store 分别重解析 durable objects并验证 Pi-shaped candidate 可由 envelope 确定派生。独立本地 harness 通过真实 Pi 0.84.1 `Models.complete`（非 Simple）与 OpenAI SDK 6.26.0 捕获一次 fake fetch，逐字证明 candidate body、`max_completion_tokens` 与 retry-count；builder/plan/composition 不继承该 proof，仍标 per-request proof pending。当前 Node 22.13.0 低于声明的 >=22.19.0，且真实 endpoint/account、redirect policy、SSE response capture、exactly-once、model tokenizer、rights、外部可信 toolchain capsule、签名/WORM/head 与盲评统计仍未完成，故 `api_execution_allowed=false` |
