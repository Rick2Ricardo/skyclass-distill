...
>
> 2026-08-12 Formal Oracle 私有运行仓库补充：两个薄片现已实现 `0700/0600` 内容寻址对象、owner-nonce 跨进程锁、create-once HEAD、external HEAD pin、连续 checkpoint、durable dispatch 恢复，以及 attempt receipt、raw→parsed、共享结构 schema、safe retry/terminal 与 `SCHEMA_VALIDATED_COMMITTED`。结构调度与实际执行计划使用两个独立哈希锚：前者强制每个 case 完整覆盖四臂×全部预注册 seeds，后者逐请求冻结渲染后 prompt、视觉引用、request payload 字节哈希及全部预算/协议字段；提交时 plan、intent 与真实 bytes 必须一致。`EXECUTION_COMPLETE` 只说明整个冻结矩阵完成 transport/schema 持久化，语义状态保持 `pending_external_blind_review`。本层仍不证明 case 是当前 Gold 全集，也不能防止本地同 UID 同时回退 HEAD 与外部旧 pin；必须由下一层进程外 pinned registry/composition + 单调/WORM HEAD 关闭这些边界，API 继续为 false。
...
| Formal Oracle structural + ledger + media + frame + private run-store + composition/request-envelope/prompt-renderer/Pi-body/local-SSE slices | PASS only as non-executable composed infrastructure | externally pinned composition 已绑定 generation-0 HEAD、`SEALED_READY` 与全 `PENDING`。strict envelope 与 deterministic prompt 从 verified bytes 重建；plan/intent v2 拆分 envelope/body 双 hash，Store 分别重解析 durable objects并验证 Pi-shaped candidate 可由 envelope 确定派生。独立本地 harness 通过真实 Pi 0.84.1 `Models.complete`（非 Simple）与 OpenAI SDK 6.26.0 捕获一次 fake fetch，逐字证明 candidate body、`max_completion_tokens` 与 retry-count；同一个内存 raw SSE 又经 strict multi-delta/finish/usage/DONE parser 在 Pi 消费前后重算，绑定 raw/content hash、identity、stop、usage 与双请求 hash。该 response proof 只覆盖 internal-memory fake fixture，Store/composition 未消费且仍 pending。当前 Node 22.13.0 低于声明的 >=22.19.0，真实 endpoint/account、redirect、provider SSE capture、exactly-once、model tokenizer、rights、外部可信 toolchain capsule、签名/WORM/head 与盲评统计仍未完成，故 `api_execution_allowed=false` |
