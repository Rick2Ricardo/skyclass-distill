...
2. 双标 `kg005 1888–1905 s` 的 ERASE+ADD 困难负例，把最后一个操作长尾纳入人工仲裁；随后只用真正签字的事件编译内容寻址 `signed-gold-dataset-v1`，再由其中的单课包进入时序多模态 Skill 蒸馏；当前 2 条工程 Gold-dev 只用于闭环测试；
