...
> 当前阶段：两位教师已有 6 个独立 A/B（含严格盲标 B2）待签仲裁包；李永乐后半段 7 组（提出 9 个候选事件）、坤哥主窗 19 组、李永乐假 MODIFY 困难负例 17 组、坤哥同槽重写 5 组、坤哥真实 ERASE 1 组、坤哥 ERASE→ADD 困难负例 3 组，共 52 个 review groups、54 个候选事件，仍不能冒充论文 Gold
...
>
> 2026-08-12 补充：产品新增 `Gold 仲裁` 中心，直接读取 canonical clip manifest 中登记的 6 个 A/B 仲裁输入并统一为 52 组、54 个候选事件、379 份视觉证据的评审队列。每组展示全部冻结 A/B 事件、全部原始视觉证据、ASR 上下文与未决字段；人工决策以 append-only 修订链保存，绑定 intake SHA-256、父版本和记录哈希。只有包内每组都有决策后才能开始包级签字；第一位视觉仲裁人签字即锁定决策，第二位不同人员以物理复核角色签字后整包正式冻结。证据按登记索引、真实图片解码与 SHA-256 fail-closed 提供。当前仍是 `0/52` 已裁决、`0/6` 双签冻结，不能生成或宣称论文 Gold。
>
> 2026-08-14 Signed Gold v2 接线补充：当前 6 个不可变双签 component packages 确定性聚合为 4 个 lesson；同 source 的多个组件不再被错误拒绝，也不伪合并签字。每个视觉资产保留 `source_evidence_id`，同时按 package/group/kind/path/hash 派生唯一资产 ID。真实队列结构预检为 6 components、4 lessons、52 groups、379 assets、52 个唯一 canonical comparisons，结构已可编译；人工状态仍为 `0/52` 裁决、`0/6` 双签和 `0/30` 接受事件，因此正式编译与单课运行继续 fail-closed。
>
> 2026-08-12 Formal Oracle Gate 补充：已落地 30–50 个签字事件、至少两位教师、四种 operation、至少三 seeds 的确定性结构预检与 `case × 4 arms × seed` 调度。该层只返回 `untrusted_structure_valid`，并固定 `api_execution_allowed=false`；它只能证明输入在结构上自洽，不能证明输入来自当前可信评审账本或冻结预注册。真实 API 继续被锁住，直到完成账本重编译证明、不可变预注册、媒体/ASR 字节校验、私有内容寻址运行仓库、断点续跑和盲评统计闭环。
>
...
>
> 2026-08-12 Formal Oracle 私有运行仓库补充：现已实现 `0700/0600` 内容寻址对象、owner-nonce 跨进程锁、create-once HEAD、external HEAD pin、连续 checkpoint、durable dispatch 恢复，以及 v3 attempt receipt 的 SSE A/B/C/D 四层持久化、共享结构 schema、safe retry/terminal 与 `SCHEMA_VALIDATED_COMMITTED`。成功 receipt 把 fetch-observed SSE、domain-addressed derivation、assistant-content UTF-8 bytes（`assistant-content.utf8`）与 canonical response/commitment 分开；提交只接受 runtime-branded artifact，历史重载从 raw SSE 重派生并验证其余三层、双请求根、completion ID/model/stop/usage/budgets。结构调度与实际执行计划继续使用两个独立哈希锚。`EXECUTION_COMPLETE` 只说明整个冻结矩阵完成 SSE/transport/schema 持久化，语义状态保持 `pending_external_blind_review`。本层仍不证明 case 是当前 Gold 全集，也不能防止本地同 UID 同时回退 HEAD 与外部旧 pin；必须由下一层进程外 pinned registry/composition + 单调/WORM HEAD 关闭这些边界，API 继续为 false。

> 2026-08-12 Formal Oracle composition/request-envelope/prompt-renderer 补充：composition 按 `registry → Gold ledger → media/ASR/frame → run` 固定锁序组合全部根与 create-once generation-0 HEAD。shared strict canonical future-Pi-adapter envelope 逐字段冻结 model、prompt、canonical image base64/label、seed、temperature、预算、cache/tools 和 hidden retries=0；Store 仅收 branded artifact并在提交/重载时重解析。新增 `teacher-evidence-user-prompt-v1` 固定代码模板、exact placeholders 与 `teacher-evidence-response-v1` schema；composition 从 byte-preflight verified selected transcript 重建 prompt，不再接收 caller rendered bytes。四臂共享相同 transcript/task/rules/schema，只让中性 `visual-1` availability 与真实视觉输入变化。该 envelope 仍不是 provider wire，provider adapter/account/endpoint、model tokenizer、rights active head、toolchain capsule、composition 外部签名/WORM、external monotonic HEAD、blind package 和 statistics pending，API 保持 false。
...
| Oracle pilot clip manifest | PASS for annotation intake | 已冻结 11 个片段、6 个源视频、2 位教师、两种板书媒介；6 个片段完成独立 A/B（含严格盲标 B2）与待签对齐，其余保持候选。全部为 `internal_review_only` 和 `needs_review`，尚无论文级 accepted delta |
...
| ERASE→ADD hard negative | PASS for human review only | `kg005 / 1888–1905 s` 已完成严格独立 A/B 与 3 组对齐；前两组一致支持 `ERASE → ADD`，第三组因片尾右删失保持 A=`unknown`、B=`ERASE`。3/3 pending，accepted=0；不得把空间复用自动合并成 MODIFY，也不得把片尾不完整 ERASE 自动补真 |
| Human Gold review workflow | PASS as unsigned review infrastructure | `/api/gold-review` 与产品 `Gold 仲裁` 中心已统一载入 6 个包/52 组/54 个候选事件；379/379 份图像按冻结 hash 解码并全部展示，组决策 append-only、按包串行写入且父链可验。全组裁决后由视觉仲裁与物理复核两名不同人员双签；首次签字锁定决策，双签才正式冻结。当前无人工决策或签字，因此仅证明评审基础设施可用，不证明 Gold 已完成 |
| Signed Gold compiler and single-lesson handoff | PASS as gated infrastructure | 只有 `paper_gold_ready` 后才可编译确定性 `signed-gold-dataset-v2`；输出位于内容寻址目录，运行时重新验证计数、双签身份、决策覆盖、真实 comparison 图与规范 payload hash。产品可从已编译数据集选择一节双签课进入真实多模态 Skill 蒸馏，视觉请求按精确证据 ID 审计。当前 0 个包双签，所以尚无正式 Signed Gold 文件、正式单课 Skill 或价值结论 |
| Operation-gap scouting | PASS for human review intake | `kg005 2134–2166 s` 的初始 MODIFY 假设已被双标否定并转为困难负例；`kg003 4422–4428 s` 已形成严格 A/B2 ERASE 仲裁输入；`kg005 1888–1905 s` 已完成 A/B 与待签对齐。本地 ASR 已生成，三者均未进入 Gold |
| Formal Oracle structural + ledger + media + frame + private run-store + composition/request-envelope/prompt-renderer/Pi-body/local-SSE + transport authority slices | PASS only as non-executable composed infrastructure | externally pinned composition 已绑定 generation-0 HEAD、`SEALED_READY` 与全 `PENDING`。strict envelope 与 deterministic prompt 从 verified bytes 重建；plan/intent v2 拆分 envelope/body 双 hash。composition 现对 execution plan 的每个请求都通过真实 Pi 0.84.1 `Models.complete`（非 Simple）与 OpenAI SDK 6.26.0 运行内部 `example.invalid` fake-fetch proof，闭合双 hash、单次 fetch、hidden retry=0、compat、完整 SSE 与统一兼容 Node runtime；proof-set 按 `schedule_index` 保存完整 domain-addressed Pi/SSE proof，并以受信 execution plan cross-validator 逐项闭合 request/model/input-output budgets，拒绝倒序、删重、错配和任意 proof root。Run Store 的 breaking attempt v4 / commit v3 已接入 branded response artifact：成功以 A/B/C/D 四层不可变对象持久化；完整 entity 在 transport metadata、SSE、assistant JSON 或 arm schema 任一阶段失败则直接不可重试 `FAILED_CLOSED`，并可跨重启重验。新增 transport authority registry 用进程外 Ed25519 signer 冻结当前 ledger/composition/run/plan/model 与 canonical HTTPS endpoint、account/credential key ID、redirect/retry/unknown策略，支持时间窗、create-once撤销和 callback 临时能力；能力不含 secret。single-consume sender 已要求 durable HEAD-CAS 私有 lease、callback secret、全部 public DNS answer、one-shot HTTPS socket 与实际 remote IP/family 闭合，并持久化 authoritative complete/partial/no-header capture。新增 strict input-token receipt/set 合同与 callback-only 结构绑定能力，逐请求闭合 schedule/request/model/envelope/body/budget/count/capture/authority roots；本薄片不接收或持久化 count entity bytes/URI，故这些调用方声明的 hash/length 只构成 structural metadata binding，不证明 capture byte authenticity。官方 `/responses/input_tokens` receipt 与当前 Pi Chat Completions provider body 又是不同请求实体，composition v3 只能记录 transport-incompatible receipt infrastructure，不能解除当前 input budget pending。`.node-version` 固定 Node 22.23.2/最低22.19.0；Chat Completions 同路径 exact count authority、rights、外部可信 toolchain capsule、endpoint/account active-head WORM、正式批量 execution gate与盲评统计仍未完成，故 `api_execution_allowed=false` |
| Formal Oracle preregistration V2 roots + composition V4 + execution lineage V2 | PASS as non-executable transport/schema infrastructure only | 无哈希环 DAG `Evidence Policy V2 + Statistics Plan V2 → Formal Spec V2 → Rating Plan V2 → Run Contract V2` 已冻结并在 generation-0 前持久化；composition V4 按 registry→Gold ledger→byte/ASR/frame→Store 重跑并原子提交 exact genesis。新 execution migration 不复制 genesis、不建立第二 HEAD，而在同一 owner-nonce 锁和 mandatory external gen0 pin 下把唯一 HEAD CAS 到 generation-1 V2；此后仅接受 domain-separated IntentV3/AttemptV5/CommitV4/CheckpointV2/TerminalV2/HeadPinV2。Store 已实现 durable single-consume dispatch、authoritative capture、confirmed no-result retry、unknown/invalid immediate terminal、A/B/C/D 每次重载重派生、12-request `EXECUTION_COMPLETE` 及 callback-only completed chain；旧 V1 lineage 不能混入。真实 V2 evidence-source/rating/统计交接、外部 monotonic/WORM、rights/toolchain/正式 orchestrator 均仍 pending，`api_execution_allowed=false`，不产生真实 API、分数或 GO/STOP。 |
...
1. 在产品 `Gold 仲裁` 中心对当前六个仲裁包的 52 个 review groups（54 个候选事件、379 份视觉证据）逐组裁决并执行包级签字；任何 proposal、单标或 Agent 对齐结果均保持 `needs_review`，不能用“候选数超过 50”冒充“50 条 Gold”；
2. 对 `kg005 1888–1905 s` 的前两组显式裁决 `ERASE → ADD`，对右删失第三组保留 `unknown` 或由人审给出有证据的最终事件；随后只用真正签字的事件编译内容寻址 `signed-gold-dataset-v2`，再由其中的单课包进入时序多模态 Skill 蒸馏；当前 2 条工程 Gold-dev 只用于闭环测试；
3. breaking V2 execution lineage 已在同一唯一 HEAD 上完成 migration、dispatch/receipt/retry/terminal 与 12-request transport/schema completion，不与 V1 intent/audit/commit/checkpoint 混用；下一步是把 V2 completed callback 能力接入可信 post-run evidence-source、rating assignment/ledger/set/report/compiler V2，并补外部 monotonic/WORM 与正式批量 orchestrator。在此之前 formal compiler 继续硬拒、`api_execution_allowed=false`、所有实验结果保持 `TBD`；
