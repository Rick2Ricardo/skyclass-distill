...
> 2026-08-12 Formal Oracle composition/request-envelope 补充：composition 按 `registry → Gold ledger → media/ASR/frame → run` 固定锁序，把 externally pinned Ed25519 registry capability、当前 Signed Gold snapshot、formal manifest/spec/resource root、结构 schedule、媒体/签字 ASR、source-frame proofs、run/execution-plan 与 create-once generation-0 HEAD 原子组合。其上已实现 shared strict canonical future-Pi-adapter envelope builder/parser：逐字段冻结 model、prompt bytes/hash、canonical image base64/label、seed、temperature、token/time/attempt budgets、cache/tools，并固定 hidden provider retries 为 0；Store 仅收 branded artifact，并在提交/重载时重解析真实 bytes。该 envelope 不是 provider wire，template grammar/renderer、provider adapter/account/endpoint、rights active head、toolchain capsule、composition 外部签名/WORM、external monotonic HEAD、blind package 和 statistics 仍 pending，API 保持 false。
...
| Formal Oracle structural + ledger + media + frame + private run-store + composition/request-envelope slices | PASS only as non-executable composed infrastructure | 结构、可信账本、媒体/语音、source-frame 与 run-store 已由 externally pinned composition 按固定锁序组合，并绑定 generation-0 external HEAD pin、`SEALED_READY` 与全 `PENDING`。strict canonical future-adapter envelope 已逐字段闭合 plan/spec/verified visual bytes，Store 只收 branded artifact并在 durable reload 时重解析；但 user-template→rendered-prompt derivation、provider wire/account/endpoint、authoritative rights/withdrawal head、不可变 toolchain capsule、composition 外部签名/WORM、不可回滚 external HEAD、盲评统计仍未完成，因此 `api_execution_allowed=false` |
...
3. 下一步冻结 user-template grammar/renderer/version 与 placeholder exact set，从 verified case transcript/arm/schema 确定性派生 rendered prompt；随后实现只接收 branded envelope 的 prepared transport adapter，并证明 captured provider body 等价，同时绑定 provider account/endpoint。再接 authoritative rights/withdrawal head、不可变 toolchain capsule、composition 外部签名/WORM、external monotonic HEAD、私有答案键、公共盲评包与统计器。teacher-only 违规和无证据学生结果必须原样保留为盲评失败样本，不能由运行存储选择性过滤。全部通过后才可能由独立执行 gate 解除 `api_execution_allowed=false`；所有结果继续保持 `TBD`；
