...
| Formal Oracle structural + ledger + media + frame + private run-store slices | PASS only as separately gated infrastructure | 结构层冻结 30–50 events、至少两位教师、四种 operation、至少三 seeds 与完整 schedule；可信账本层重编译 Signed Gold 并由外部 Ed25519 registry 固定；媒体/语音层闭合真实字节，frame 层证明 Static/Uniform 来自声明视频 PTS；run-store 两个薄片已经冻结完整四臂×seed 矩阵、逐请求执行计划、真实 request/response bytes、strict raw→parsed JSON、共享 arm schema、attempt receipt、retry/fail-closed、`SCHEMA_VALIDATED_COMMITTED` 和连续 checkpoint/HEAD 恢复。`EXECUTION_COMPLETE` 只表示矩阵内所有请求的传输与结构闭环，不表示教学语义或 teacher-only 审查通过。上述能力尚未由一个外部 pinned composition gate 原子组合，盲评统计和不可回滚外部 HEAD 仍未完成，因此 `api_execution_allowed=false` |
...
3. 下一步把 pinned ledger registry、media/ASR、source-frame derivation 与私有 run-store 两个薄片组合成单一 fail-closed capability；再完成 external monotonic/WORM HEAD、私有答案键、公共盲评包与统计器。teacher-only 违规和无证据学生结果必须原样保留为盲评失败样本，不能由运行存储选择性过滤。全部通过后才可能由独立执行 gate 解除 `api_execution_allowed=false`；所有结果继续保持 `TBD`；
