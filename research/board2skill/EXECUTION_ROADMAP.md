...
>
> 2026-08-12 Formal Oracle 私有运行仓库补充：已实现第一薄片的 `0700/0600` 内容寻址对象、owner-nonce 跨进程锁、create-once HEAD、external HEAD pin、连续 checkpoint 与 durable dispatch 恢复。结构调度与实际执行计划使用两个独立哈希锚：前者强制每个 case 完整覆盖四臂×全部预注册 seeds，后者逐请求冻结渲染后 prompt、视觉引用、request payload 字节哈希及全部预算/协议字段；提交时 plan、intent 与真实 bytes 必须一致。已有 HEAD 禁止再次 create，durable dispatch 恢复时只能阻断，不能自动重发。本层仍不证明 case 是当前 Gold 全集，也不能防止本地同 UID 同时回退 HEAD 与外部旧 pin；必须由下一层进程外 pinned ledger registry + 单调/WORM HEAD 组合门关闭这些边界，API 继续为 false。
...
| Formal Oracle structural + ledger + media + frame + private run-store slices | PASS only as separately gated infrastructure | 结构层冻结 30–50 events、至少两位教师、四种 operation、至少三 seeds 与完整 schedule；可信账本层重编译 Signed Gold 并由外部 Ed25519 registry 固定；媒体/语音层闭合真实字节，frame 层证明 Static/Uniform 来自声明视频 PTS；run-store 第一薄片冻结完整四臂×seed 矩阵、逐请求执行计划、真实 request bytes 和 checkpoint/HEAD 恢复。上述能力尚未由一个外部 pinned composition gate 原子组合，完整 attempt receipt/verified commit、盲评统计和不可回滚外部 HEAD 仍未完成，因此 `api_execution_allowed=false` |
...
3. 下一步把 pinned ledger registry、media/ASR、source-frame derivation 与私有 run-store 第一薄片组合成单一 fail-closed capability；再完成 attempt receipt、verified commit、external monotonic/WORM HEAD、私有答案键、公共盲评包与统计器。全部通过后才可能由独立执行 gate 解除 `api_execution_allowed=false`；所有结果继续保持 `TBD`；
