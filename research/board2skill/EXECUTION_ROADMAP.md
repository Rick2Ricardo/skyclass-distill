...
| Formal Oracle structural + ledger + media + frame + private run-store + composition/request-envelope/prompt-renderer/provider-body serialization slices | PASS only as non-executable composed infrastructure | externally pinned composition 已绑定 generation-0 HEAD、`SEALED_READY` 与全 `PENDING`。strict envelope 与 deterministic prompt 从 verified bytes 重建；plan/intent v2 又拆分 envelope/body 双 hash，Store 分别重解析 durable objects并验证候选 OpenAI-compatible body 可由 envelope 确定派生。该 body profile 与 frozen `transport=pi` 明确标为 `not_dispatchable_transport_mismatch`；fake fetch 仅可到 `example.invalid`，不构成生产 sender。Pi/direct profile selection、真实 endpoint/account/wire、SSE response capture、exactly-once、model tokenizer、rights、toolchain capsule、外部签名/WORM/head 与盲评统计仍未完成，故 `api_execution_allowed=false` |
...
3. 下一步先选择并冻结唯一可执行 transport profile：若保留 Pi，必须对 0.84.1 + OpenAI SDK 6.26.0 做真实 fetch-boundary capture、compat/token/store/tool 字段证明且禁 onPayload/hidden retry；若采用 direct fetch，则迁移 formal spec 的 `transport` 并重新生成 plan/run roots。随后实现只接收 branded prepared artifact 的生产 sender，绑定 authoritative endpoint/account/idempotency、禁止 redirect、timeout→unknown receipt、严格 SSE raw bytes/usage/stop capture，并以 model-specific tokenizer/image accounting 关闭 input budget 门。再接 rights/withdrawal head、不可变 toolchain capsule、composition 外部签名/WORM、external monotonic HEAD、私有答案键、公共盲评包与统计器。全部通过后才可能由独立 execution gate 解除 `api_execution_allowed=false`；所有结果继续保持 `TBD`；
