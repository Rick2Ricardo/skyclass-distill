...
3. 已在兼容 Node >=22.19 的冻结入口中把逐请求 local fake-fetch proof 纳入受控 composition，并以独立 Ed25519 transport authority registry 绑定 canonical HTTPS DNS-name endpoint 声明、provider/account/credential ID、redirect/retry/unknown策略和当前 composition/run/plan roots。现已继续接入 durable HEAD-CAS 后才出借的 callback-scoped single-consume lease、外部 callback secret、运行时全量 DNS answers 公网校验与固定 IP/TLS hostname lookup、零 SDK retry、精确 Pi body CAS、完整/partial/no-header fetch entity capture，以及 Store 的 capture record + 原始 entity 私有内容寻址持久化和重启重验。完整 SSE 能生成 strict result 或 invalid-response artifact；partial/no-header 一律进入 unknown/block，DNS 私网或解析失败发生在发网前。input-token receipt infrastructure 已能严格绑定未来 Responses API exact-count receipt，但不把它翻译为当前 Chat Completions 精确计数；当前预算门仍 pending。私有答案键、公共盲评/证据合同、双评分账本和配对统计编译器现已完成结构闭合，但真实 public-evidence 生成、外部盲评会话与 key registry、签字/WORM 和真实数值仍 pending。该 sender 仍只返回 `api_execution_allowed=false` 的 capture，不是正式批量 execution capability；下一步必须取得与当前实际 provider body 同一路径的 exact count authority，或 breaking 迁移 transport/envelope/body 到同一 Responses 请求实体，再接 rights/withdrawal head、外部可信不可变 toolchain capsule、transport/composition active-head WORM 及上述真实盲评门。全部通过后才可能由独立 execution gate 解除 false；所有实验结果继续保持`TBD`；
