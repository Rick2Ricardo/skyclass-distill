...
| Formal Oracle structural + ledger + media + frame + private run-store + composition/request-envelope/prompt-renderer/Pi-body/local-SSE slices | PASS only as non-executable composed infrastructure | externally pinned composition 已绑定 generation-0 HEAD、`SEALED_READY` 与全 `PENDING`。strict envelope 与 deterministic prompt 从 verified bytes 重建；plan/intent v2 拆分 envelope/body 双 hash。独立本地 harness 通过真实 Pi 0.84.1 `Models.complete`（非 Simple）与 OpenAI SDK 6.26.0 捕获一次 fake fetch，逐字证明 candidate body；同一个内存 raw SSE 经 strict parser 绑定 identity/stop/usage 与请求双根。Run Store 的 breaking attempt/commit v3 已接入 branded response artifact：成功以 A/B/C/D 四层不可变对象持久化；完整 entity 在 SSE/assistant JSON/arm schema 任一阶段失败，则以 A 加可选 B/C 和独立 invalid record 直接持久化为不可重试 `FAILED_CLOSED` terminal receipt。所有 durable reload 均从 raw SSE 重派生并拒绝层替换；但 composition 尚未逐请求运行 proof，当前 Node 22.13.0 也低于声明的 >=22.19.0。真实 endpoint/account、redirect、authoritative provider response capture、exactly-once、model tokenizer、rights、外部可信 toolchain capsule、签名/WORM/head 与盲评统计仍未完成，故 `api_execution_allowed=false` |
...
3. 下一步在兼容 Node >=22.19 的冻结环境中，把当前逐请求 local fake-fetch proof 纳入受控 composition/execution gate，并实现只接收 branded prepared artifact 的生产 sender：绑定 authoritative endpoint/account/idempotency，补禁止 redirect、timeout→unknown receipt、严格 SSE raw bytes/usage/stop capture，以 model-specific tokenizer/image accounting 关闭 input budget 门。再接 rights/withdrawal head、外部可信不可变 toolchain capsule、composition 签名/WORM、external monotonic HEAD、私有答案键、公共盲评包与统计器。全部通过后才可能由独立 execution gate 解除 `api_execution_allowed=false`；所有结果继续保持 `TBD`；
