...
3. 已在兼容 Node >=22.19 的冻结入口中把逐请求 local fake-fetch proof 纳入受控 composition，并以独立 Ed25519 transport authority registry 绑定 canonical HTTPS DNS-name endpoint 声明、provider/account/credential ID、redirect/retry/unknown策略和当前 composition/run/plan roots；registry 不把 DNS 字面校验冒充运行时 IP 来源证明。下一步在 transport→run 固定锁序内实现只接收 branded prepared artifact 与外部 callback secret 的 single-consume sender，在每次发送前闭合 DNS resolution、IP allow/deny、rebinding 与 redirect policy，并把 durable dispatch intent、timeout/网络不确定→unknown receipt、fetch-observed SSE raw bytes/usage/stop capture原子闭合；同时以 model-specific tokenizer/image accounting关闭input budget门。再接rights/withdrawal head、外部可信不可变toolchain capsule、transport/composition active-head WORM、私有答案键、公共盲评包与统计器。全部通过后才可能由独立execution gate解除`api_execution_allowed=false`；所有结果继续保持`TBD`；
