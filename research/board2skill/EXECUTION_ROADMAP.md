...
>
> 2026-08-12 Signed Gold 接线补充：双签门通过后，服务端可把当前 append-only 决策编译为确定性、内容寻址的 `signed-gold-dataset-v1`；加载时重新验证至少 30 个接受事件、全部组决策覆盖、视觉/物理两名不同人员双签、签字所覆盖的决策集合、comparison 视觉元数据与数据集规范哈希。单课入口随后按“一组一张 canonical comparison 图”实际提交视觉 API，每批最多 4 图，并把精确 group/transition/delta/evidence、请求哈希、模型 stop 与 usage 写入 Skill provenance。当前人工状态仍为 `0/52` 与 `0/6`，因此编译和真实单课运行都保持 fail-closed；这只是把人工 Gold 完成后的下一步接通，绝不把候选数据提升为 Gold。
>
...
| Signed Gold compiler and single-lesson handoff | PASS as gated infrastructure | 只有 `paper_gold_ready` 后才可编译确定性 `signed-gold-dataset-v1`；输出位于内容寻址目录，运行时重新验证计数、双签身份、决策覆盖、真实 comparison 图与规范 payload hash。产品可从已编译数据集选择一节双签课进入真实多模态 Skill 蒸馏，视觉请求按精确证据 ID 审计。当前 0 个包双签，所以尚无正式 Signed Gold 文件、正式单课 Skill 或价值结论 |
...
| Formal Oracle preregistration V2 roots + composition V4 | PASS as pre-execution anchor infrastructure only | 新增无哈希环 DAG `Evidence Policy V2 + Statistics Plan V2 → Formal Spec V2 → Rating Plan V2 → Run Contract V2`，冻结指标顺序、单一 baseline/tie、层级聚合/bootstrap、single-event temporal 对称 NA 与空 temporal population 阻断规则。独立 create-once 私有 Store 在 generation-0 前逐份持久化并重读 policy/statistics/spec/rating/run/schedule/execution-plan/checkpoint 正文，外部 HEAD pin 回滚与任一正文漂移 fail closed。composition V4 在 registry→Gold ledger→byte/ASR/frame→Store 固定顺序下重跑已有字节/媒体/Pi 证明，并在 Store owner-nonce 锁内原子 create+reload actual V2 genesis pin；借出的 V2 capability 仅 callback 内有效，record 仍 non-authoritative。V1 字节/domain/validator 保持只读兼容，旧 run 不得补根升级。V2 dispatch/resume、可信 evidence-source 交接、rating assignment/ledger/set/report/compiler V2、外部签名/WORM 仍 pending，因此该 snapshot 明确不可执行且不产生真实统计或 GO/STOP。 |
...
2. 对 `kg005 1888–1905 s` 的前两组显式裁决 `ERASE → ADD`，对右删失第三组保留 `unknown` 或由人审给出有证据的最终事件；随后只用真正签字的事件编译内容寻址 `signed-gold-dataset-v1`，再由其中的单课包进入时序多模态 Skill 蒸馏；当前 2 条工程 Gold-dev 只用于闭环测试；
3. 已冻结 breaking V2 的 Evidence Policy、Statistics Plan、Formal Spec、Rating Plan 与 Run Contract 根，并将正文与 generation-0 genesis create-once 私有持久化；composition V4 现已将同一 bundle 与 registry/Gold/media/frame/Pi proof/actual genesis pin 在 Store 锁内原子闭合。旧 V1 run 只能继续作为 post-hoc development 数据，不能补根升级。下一步是实现不可与 V1 混用的 V2 dispatch/receipt/terminal execution pipeline，再完成 rating assignment/ledger/set/report/compiler V2 和可信 evidence-source 交接；在此之前 formal compiler 继续硬拒、`api_execution_allowed=false`、所有实验结果保持 `TBD`；
