...
| Formal Oracle preregistration V2 roots | PASS as pre-execution anchor infrastructure only | 新增无哈希环 DAG `Evidence Policy V2 + Statistics Plan V2 → Formal Spec V2 → Rating Plan V2 → Run Contract V2`，冻结指标顺序、单一 baseline/tie、层级聚合/bootstrap、single-event temporal 对称 NA 与空 temporal population 阻断规则。独立 create-once 私有 Store 在 generation-0 前逐份持久化并重读 policy/statistics/spec/rating/run/schedule/execution-plan/checkpoint 正文，外部 HEAD pin 回滚与任一正文漂移 fail closed。V1 字节/domain/validator 保持只读兼容，旧 run 不得补根升级。V2 dispatch/resume、可信 evidence-source 交接、rating assignment/ledger/set/report/compiler V2、外部签名/WORM 仍 pending，因此该 snapshot 明确不可执行且不产生真实统计或 GO/STOP。 |
...
3. 已冻结 breaking V2 的 Evidence Policy、Statistics Plan、Formal Spec、Rating Plan 与 Run Contract 根，并将正文与 generation-0 genesis create-once 私有持久化；旧 V1 run 只能继续作为 post-hoc development 数据，不能补根升级。下一步把 V2 execution pipeline 与 composition 接入同一预注册 bundle，再完成 rating assignment/ledger/set/report/compiler V2 和可信 evidence-source 交接；在此之前 formal compiler 继续硬拒、`api_execution_allowed=false`、所有实验结果保持 `TBD`；
