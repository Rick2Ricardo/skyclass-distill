...
- endpoint/account authority 第一层现已新增独立的 Ed25519 signed、内容寻址 `formal-oracle-transport-registry-v1`：它精确绑定 ledger registry、composition、run、execution plan、model、canonical HTTPS DNS-name endpoint 声明、provider/account/credential key ID、禁止 redirect、零 provider retry、Store 独占 retry、chat-completions 无 provider idempotency时的 fetch 后 unknown 永久阻断策略，并以 issued/expires 时间窗和同 signer create-once revocation fail closed。registry/撤销文件保持私有 `0700/0600`、拒绝 symlink/hardlink/宽权限；读取时持有 transport registry 跨进程锁直到 callback 结束。借出的 runtime-frozen capability 不可序列化、不含 API secret，只写 `endpoint_account_attested_only`。此层只校验 DNS 形式声明，不能证明运行时解析结果；每次发送前的 DNS 解析、IP allow/deny、rebinding 防护和 redirect 复核仍由 future sender 负责。真实 credential、single-consume sender、真实 wire/SSE capture、可信 toolchain capsule与 rollback-proof active head 均未完成，所以 `api_execution_allowed=false`。
