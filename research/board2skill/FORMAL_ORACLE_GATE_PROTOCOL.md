...
- 根输入只能是重新验证签字正文、内容哈希与视觉证据的 `signed-gold-dataset-v2`；v2 保留人工双签 component packages，并按 source video 确定性派生 lesson 聚合与逐资产唯一 ID。
...
- post-run evidence source gate 只在 `registry → Gold ledger → byte/ASR → frame → terminal run lock` 同一 callback 链中出借临时能力。它要求 exact external terminal HEAD pin、`EXECUTION_COMPLETE` 与全部结构提交，从 durable A 重新派生 B/C/D，再以 domain-separated secret commitment/HMAC 重建 blind artifacts；claims 只来自 D 的固定 JSON-pointer 投影，speech 只来自本次签名 ASR 重验的 selected transcript（`context_not_gold`），board evidence 只来自当前 signed-gold-dataset-v2 `final_events` 的固定 semantic projection。持久 public-evidence-v2/receipt 自哈希仍非真实性证明；该层只支持 `post_hoc_development_only`，不生成评分、统计或 GO/STOP。
...
- input-token count receipt 基础设施现使用独立 domain-addressed `formal-oracle-input-token-count-receipt-v1` 与稠密 receipt-set：每个条目精确绑定 `schedule_index/request_id/model/request_envelope_sha256/provider_body_sha256/max_input_tokens/exact_input_tokens`、计数请求/响应 capture 根及 authority/profile/version；缺失、重复、乱序、任一根或模型漂移、非安全整数及超预算均 fail closed。当前薄片不接收、读取或持久化 count request/response entity bytes 或 URI，只核对调用方声明的 hash 与 length；因此自洽但伪造的 metadata 仍可能获得仅限 callback 的 structural-only brand，capture roots 绝不表示 byte authenticity。持久 receipt/set 明确是 `non_authoritative_*`，自哈希不证明 endpoint、account、签名或 WORM；临时 runtime brand 仅在校验 callback 内有效且不授权 API。官方精确计数端点是 `POST /responses/input_tokens`，它精确对应 Responses API 输入实体；当前冻结执行实体仍是 Pi 0.84.1 Chat Completions body，因此 composition v3 最多记录 `responses_exact_count_receipts_bound_transport_incompatible`，真实 `input_token_budget_status` 固定为 `pending_exact_chat_completions_count_authority`。本地 tokenizer 也不得冒充图片和消息边界的 provider 精确计数。内存 authority 只用于无网络测试；只有未来 transport breaking 迁移到同一 Responses 请求实体，或取得当前 Chat Completions 同路径的权威计数 receipt，才可另行升级预算门。
...
breaking 预注册 V2 使用无环依赖：`PolicyV2 + StatisticsPlanV2 → FormalSpecV2 → RatingPlanV2 → FormalRunContractV2`。Spec 不反向引用 RatingPlan；Run 同时锚定 bundle/policy/statistics/spec/rating 五根。StatisticsPlan 明文冻结 baseline/tie、两评分者等权、case-seed→case→video→teacher 聚合、层级配对 bootstrap、single-event temporal 对称 NA 与空 temporal population 阻断。create-once 私有 Store 在 generation-0 HEAD 提交前发布并重读所有真实正文以及 schedule/execution plan/checkpoint；单独自报 SHA 不成立。composition V4 现以同一容器内的 `registry → Gold ledger → byte/ASR/frame → preregistration Store` 顺序重跑全链，将 bundle 五根、V2 run、双请求根、local Pi proof 与 actual generation-0 pin 闭合；Store 在 owner-nonce 锁仍持有时 create、reload 并借出 callback-only capability。随后同一个 `run_store_uri/runs/<run>/HEAD` 可在 mandatory external generation-0 pin 下做一次显式 migration：generation 1 起仅接受 breaking `RequestIntentV3`、`RequestAttemptAuditV5`、`CommittedRequestV4`、`RunCheckpointV2`、`TerminalReasonV2` 与 `HeadPinV2`，不得复制 genesis、另建第二个 HEAD 或混入 V1 lineage。V2 Store 已实现 durable dispatch lease、authoritative sender capture、confirmed no-result retry、unknown/invalid fail-closed、A/B/C/D 重派生、12-request terminal completion 与 callback-only completed transport/schema chain；每次 inspect/load 都重读完整 checkpoint 历史和全部可达对象。该能力仍只证明 transport/schema 链，语义审查、V2 evidence/rating/statistics、外部 monotonic/WORM 和正式 execution orchestrator 继续 pending，所有接口固定 `api_execution_allowed=false`，所以绝不是正式 API 执行能力。V1 仍使用原 schema/domain/canonical bytes，已完成 V1 run 不得补根升级。
