...
- `proveNonProductionFormalOraclePiFetchBoundary` 使用真实本地 Pi `Models.complete`（明确禁止 `completeSimple`）与 `openAICompletionsApi`，固定 compat/cache/session/tools/reasoning/sampling/retry 参数，把调用送入模块内部 `example.invalid` guard。exported proof 不接收 caller fetch 或 API key，也不读取环境 credential；它只用固定 dummy credential 与内建 fake SSE，因此自身没有网络委托面。guard 逐字比较实际 OpenAI SDK `init.body` 与持久 candidate、检查 URL/POST/runtime Authorization/SDK 版本/单次 fetch/SDK retry-count=0，`onPayload` 只能比较并返回 `undefined`，不会替换 payload；`4200` 输入预算配 `2048` 输出上限时仍捕获 `max_completion_tokens=2048`，证明未经过 Simple token clamp。fake SSE 只用于让请求路径结束，不证明缺 `[DONE]`、usage/model/数值范围/尾随 bytes 等响应语义；返回值仍固定 `provider_response_capture_status=pending_strict_sse_capture_contract`。该函数只返回独立、非执行的本地 fetch-boundary proof，不升级 builder artifact 或 composition。
- 本地记录的 Pi/OpenAI 版本与源码哈希仅叫 `observed_local_dependency_hashes`，不是可信 toolchain capsule。当前 Node `v22.13.0` 低于项目/Pi 声明的 `>=22.19.0`，所以 runtime 明确保持 `pending_incompatible_node_engine_and_external_immutable_capsule`；composition 也保持 `provider_body_transport_compatibility_status=pending_per_request_local_fake_fetch_proof`。OpenAI SDK fetch init 未绑定 `redirect:error`，因此 redirect policy 仍 pending。真实 endpoint/account、禁止重定向、外部可信 toolchain capsule、exactly-once/idempotency、timeout→unknown receipt、SSE raw response/usage/stop capture与 model-specific tokenizer/image accounting 仍未完成；加上 rights/authenticity/WORM、external monotonic head、盲包与统计，`api_execution_allowed=false`。
