...
- 已冻结浏览器安全的内容寻址 run/intents/attempt receipts/commits/checkpoints/private answer key/public blind package contracts；完整结构验证必须提交从 generation 0 到终态的连续 checkpoint 历史。breaking `RequestAttemptAuditV2` 与 `CommittedRequestV2` 使用独立 v2 domain。确认无结果只能在剩余 attempt 预算内进入 `RETRY_READY`，ambiguous 请求不得重发，attempt audit 不得重放，`SCHEMA_VALIDATED_COMMITTED` 现在证明 strict SSE derivation 加结构 schema 的 provenance 逐字段不可变，公开盲包按大小写无关方式拒绝 private answer-key 值。该状态仍不声称自由文本语义正确；teacher-only 违规、无证据学生结果等必须作为盲评失败样本保留和评分，不能在运行阶段筛掉。
- 私有运行仓库现已实现真实 `0700/0600` 内容寻址存储、跨进程 owner-nonce 锁、create-once HEAD、连续 checkpoint、强制 external HEAD pin、dispatch intent 前的不可变请求落盘，以及 v2 attempt receipt、SSE→assistant content→canonical response、结构验证与 fail-closed retry/terminal 状态。`schedule_sha256` 继续精确绑定结构预检的 8 字段 `case × arm × seed` 调度；breaking `formal-oracle-execution-plan-v2` / `oracle-gate-request-intent-v2` 分别冻结 `request_envelope_sha256` 与候选 `provider_body_sha256`。Store 在 dispatch commit 和历史重载时分别读取、严格解析并重建两份请求 bytes。成功 receipt 又必须先持久化 A fetch-observed SSE entity bytes、B domain-addressed derivation record、C derived assistant-content bytes、D canonical response bytes及其独立 public-response commitment；每次外部 reload 都从 A 重跑 parser 并逐字验证 B/C/D、completion ID、model、stop、usage、token budgets 与 intent 双根。`provider_http_request_id` 与 `completion_id` 分离；前者在未来外部 header-capture contract 落地前只是 untrusted declared metadata，不参与 A/B/C/D 或 commit 权威判断。任一 durable dispatch 在恢复时仍按 ambiguous 阻断，不能自动重发；语义审查固定为 `pending_external_blind_review`。
...
- 本地 Response 现由 `formal-oracle-pi-response-stream-v1` 严格证明：raw SSE 保留精确字节/hash，冻结 LF-only framing，允许可选唯一空 `role=assistant` prelude、随后 1..N 个非空 content delta、唯一 `stop` finish、唯一 usage-only event 与最终唯一 `[DONE]`。每帧以 duplicate-aware JSON grammar 检查 exact key set 与整数词法，但合法 JSON 空白和字段顺序不属于语义合同；id/model/created/object/index 必须全流一致，禁止多 choice、unknown/tool/reasoning、length/error、重复 usage/finish/DONE、尾随内容，usage 必须自洽且分别不超过冻结 input/output token budget。通用 runtime-branded artifact 只声明 `untrusted_sse_entity_strict_derivation_only`，不自证输入来源；外层 Pi proof 才证明测试字节来自本次模块内部 fake fetch。proof 现具有排除 self 的独立 domain hash；Store 只接受 branded artifact 提交并按上文 A/B/C/D 持久化、reload 重派生。composition 尚未逐请求运行该 proof，真实 provider endpoint/account/exactly-once/raw capture 仍 pending；invalid complete SSE 不能伪装成 unknown/no-result，本薄片仅拒绝提交，其明确 terminal receipt 尚待 transport 层实现。
- 本地记录的 Pi/OpenAI 版本与源码哈希仅叫 `observed_local_dependency_hashes`，不是可信 toolchain capsule。当前 Node `v22.13.0` 低于项目/Pi 声明的 `>=22.19.0`，所以 runtime 明确保持 `pending_incompatible_node_engine_and_external_immutable_capsule`；composition 也保持 `provider_body_transport_compatibility_status=pending_per_request_local_fake_fetch_proof` 与 `provider_response_capture_status=pending_strict_sse_capture_contract`。OpenAI SDK fetch init 未绑定 `redirect:error`，因此 redirect policy 仍 pending。真实 endpoint/account、禁止重定向、外部可信 toolchain capsule、exactly-once/idempotency、timeout→unknown receipt、真实 provider SSE capture 与 model-specific tokenizer/image accounting 仍未完成；加上 rights/authenticity/WORM、external monotonic head、盲包与统计，`api_execution_allowed=false`。
