...
- 私有运行仓库两个薄片已经实现真实 `0700/0600` 内容寻址存储、跨进程 owner-nonce 锁、create-once HEAD、连续 checkpoint、强制 external HEAD pin、dispatch intent 前的不可变请求落盘，以及 attempt receipt、raw→parsed、结构验证与 fail-closed retry/terminal 状态。`schedule_sha256` 继续精确绑定结构预检的 8 字段 `case × arm × seed` 调度；breaking `formal-oracle-execution-plan-v2` / `oracle-gate-request-intent-v2` 分别冻结 `request_envelope_sha256` 与候选 `provider_body_sha256`，彻底移除含义混杂的 `request_payload_sha256`。Store 在 dispatch commit 和历史重载时分别读取、严格解析并重建两份 immutable bytes，证明 candidate body 由 envelope 确定派生；attempt audit 的 request hash/URI 明确指 provider body。任一 durable dispatch 在恢复时一律按 ambiguous 阻断，不能自动重发；语义审查仍固定为 `pending_external_blind_review`。
...
- 新增 `openai-chat-completions-direct-serialization-v1` 候选 body：canonical JSON 只含 provider 字段，固定 system→user block 顺序、中性视觉标签与 data URL、`stream=true`、`stream_options.include_usage=true`、`max_completion_tokens`、temperature/seed、`store=false`、`tools=[]`。视觉邻接标签保留图像 SHA 以闭合内容，但不携带 run/case/arm/idempotency 等本地 provenance，API key 也绝不进入 body/持久层。这里的显式 `tools:[]` 与 Pi 0.84.1 当前 captured body（无 tools 字段）不同；Pi 还会按 provider/base URL compat 选择 token/store 字段并经 OpenAI SDK 6.26.0 组装 wire。因此该对象只可写 `provider_body_serialization_status=completed_direct_fetch_boundary_prepared`，plan/intent/artifact 必须标 `provider_body_dispatch_status=not_dispatchable_transport_mismatch`，因为 formal spec 仍冻结 `transport=pi`。`provider_body_transport_compatibility_status=pending_pi_or_direct_profile_selection`、`provider_wire_binding_status=pending_external_endpoint_account_validation`，不得把 candidate body 当成 Pi wire 或可执行 payload。
- `exerciseNonProductionFormalOraclePreparedTransportWithFakeFetch` 仅允许 `example.invalid` 与注入 fake fetch，用来断言 POST、redirect:error、runtime Authorization、timeout signal 及 `init.body` 字节等于 prepared bytes；它不是生产 sender、没有 exactly-once/idempotency capability，也不会接 Store 正式 dispatch。真实 endpoint/account、禁止重定向与 hidden retry 的运行时证明、timeout 后 unknown receipt、SSE raw response/usage/stop capture、Pi-or-direct profile selection、OpenAI/Pi/Node/build immutable capsule 均 pending。`max_input_tokens` 仍只是预算声明，model-specific tokenizer/image accounting 也 pending。加上 rights/toolchain/authenticity/WORM、external monotonic head、盲包与统计，composition 继续 `api_execution_allowed=false`。
