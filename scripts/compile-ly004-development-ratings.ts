...
import { hashOracleGateRequest, renderOracleGateUserPrompt } from "../packages/distillation/src/oracleGateRunner.js";
import type { OraclePilotArmInput } from "../packages/distillation/src/oraclePilot.js";
import { canonicalizeOracleGateCanvas } from "../packages/media/src/oracleGateCanvas.js";
import { verifyImageEvidence } from "../packages/media/src/imageEvidence.js";

const isFix = process.argv.includes("--fix");
const root = process.cwd();
const base = resolve(root, isFix ? "data/board2skill/oracle-gate-development-fix" : "data/board2skill/oracle-gate-development");
...
interface ActiveBlindItem { blind_id:string; paired_case_id:string; seed_index:number; response:Record<string,unknown>; response_sha256:string }
interface DevelopmentSpec {
  schema_version:string; protocol_scope:string; protocol_document:string; prompt_version:string;
  blind_seed_sha256:string; generation_seed:number; seed_index:number;
  cases:Array<{case_id:string;delta_id:string;uniform_frame_id:string}>;
}
interface RunManifest {
  schema_version:string; decision:string; model:string; protocol_fingerprint_sha256:string;
  prompt_sha256:string; output_schema_sha256:string; case_count:number; arm_count:number;
  seed_count:number; request_count:number; warning:string;
}
interface PrivateRunRecord extends KeyItem {
  run_id:string; response:Record<string,unknown>; response_sha256:string;
  request_audit:{request_sha256:string;model:string;attempt_count:number;provider_response_received:boolean;stop_reason:string;temperature:number;max_output_tokens:number;seed:number;cache_retention:string;transport:string;tools_policy:string;submitted_visuals:Array<{label:string;sha256:string;mime_type:string;byte_length:number}>};
  canonical_visual:null|{source_asset_uri:string;source_sha256:string;submitted_sha256:string;mime_type:string;width:number;height:number;transform:string};
}
interface PilotPackage {
  schema_version:string;bundle_id:string;
  protocol:{protocol_version:string;prompt_version:string;visual_items_per_visual_arm:number;speech_window_seconds:number;visual_budget_rule:string;runtime_pixel_and_token_audit_required:boolean};
  samples:Array<OraclePilotArmInput>;
  answer_key:Array<{blind_id:string;paired_case_id:string;case_id:string;arm:Arm;evidence_mode:string;condition_sha256:string}>;
  blind_evaluation_items:unknown[];
}
...
function parseJsonLines<T>(text:string):T[]{return text.trim().split("\n").filter(Boolean).map((line)=>JSON.parse(line) as T);}
...
function exactKeys(value:Record<string,unknown>,expected:string[],label:string):void{assert(JSON.stringify(Object.keys(value).sort())===JSON.stringify([...expected].sort()),`${label}字段不精确`);}

const expectedProtocolScope=isFix?"ly004_region_claim_decoupling_fix_gate_v1":"ly004_preregistered_development_value_gate_v1";
const expectedProtocolDocument=isFix?"research/board2skill/experiments/LY004_DEVELOPMENT_FIX_GATE_V1.md":"research/board2skill/experiments/LY004_DEVELOPMENT_VALUE_GATE_V1.md";
const expectedPromptVersion=isFix?"oracle-gate-prompt-v2-region-claim-decoupled":"oracle-gate-prompt-v1";
const expectedPromptSha=isFix?"32687e49703780c42d2c5cb582011b59d86a2faf632f8e9930ca0e38234aa2f6":"38cdf0bfad57415873b0d17572eebe592d8ec348571a38254a68dace84d5d080";
const expectedOutputSchemaSha="99ff47e5ea52494303b0acc7b6d3128b0c51a90f2451a396a60a05c5de932a11";
const expectedBlindSeedSha="3c78aab1296ba6f1f9c93f4df24df02e8982ad25fa86bcc765ef6ba6fc34bc3e";
const expectedCases=[
  {case_id:"ly004-known-condition",delta_id:"B-DELTA-05",uniform_frame_id:"GOLD-UNIFORM-01"},
  {case_id:"ly004-question-pair",delta_id:"B-DELTA-06",uniform_frame_id:"GOLD-UNIFORM-02"},
];
const bundleRoot=resolve(root,"data/board2skill/gold-dev/tbv2-ly-004-01-gold-dev-v2");
function expectedProtocolFingerprint(model:string,seed:number):string{return sha(JSON.stringify({model,prompt_sha256:expectedPromptSha,output_schema_sha256:expectedOutputSchemaSha,temperature:0,max_output_tokens:2048,cache_retention:"none",transport:"pi",tools_policy:"none",canvas:{mime_type:"image/jpeg",width:1920,height:360,quality:88},seeds:[seed]}));}
...
assert(packageManifest.schema_version===(isFix?"ly004-development-fix-blind-package-v1":"ly004-development-blind-package-v2")&&packageManifest.rater_views.join(",")==="r1,r2"&&/^[a-f0-9]{64}$/.test(packageManifest.matrix_receipt_sha256),"blind package manifest无效");
...
const activeRuns=isFix?["run-01","run-02","run-03"]:["run-01","run-03","run-04"];const specNames=isFix?["fix-seed-01","fix-seed-02","fix-seed-03"]:["seed-01","seed-03","seed-04"];const activeSeeds=isFix?[20260818,20260819,20260820]:[20260814,20260816,20260817];const activeModel="gpt-5.5";const activePromptSha=expectedPromptSha;const activeSchemaSha=expectedOutputSchemaSha;
const sourceMap:KeyItem[]=[];const activeBlindById=new Map<string,ActiveBlindItem>();const expectedSourceRoots:Array<Record<string,unknown>>=[];for(const [index,run] of activeRuns.entries()){
  const specBytes=await readFile(resolve(root,`research/board2skill/experiments/ly004-development-${specNames[index]}.json`));const specSha=sha(specBytes);const spec=JSON.parse(specBytes.toString("utf8")) as DevelopmentSpec;
  exactKeys(spec as unknown as Record<string,unknown>,["schema_version","protocol_scope","protocol_document","prompt_version","blind_seed_sha256","generation_seed","seed_index","cases"],`${run} spec`);spec.cases.forEach((item,caseIndex)=>exactKeys(item as unknown as Record<string,unknown>,["case_id","delta_id","uniform_frame_id"],`${run} spec case ${caseIndex}`));
  const allowedScope=!isFix&&index===2?[expectedProtocolScope,`${expectedProtocolScope}_replacement_block`]:[expectedProtocolScope];
  assert(spec.schema_version==="oracle-gate-smoke-spec-v1"&&allowedScope.includes(spec.protocol_scope)&&spec.protocol_document===expectedProtocolDocument&&spec.prompt_version===expectedPromptVersion&&spec.blind_seed_sha256===expectedBlindSeedSha&&spec.generation_seed===activeSeeds[index]&&spec.seed_index===(isFix?index:[0,2,3][index])&&stableJson(spec.cases)===stableJson(expectedCases),`${run} spec未满足预注册实验定义`);
  const manifestBytes=await readFile(join(base,run,"manifest.json"));const manifest=JSON.parse(manifestBytes.toString("utf8")) as RunManifest;
  ...
  assert(manifest.schema_version==="oracle-gate-smoke-manifest-v1"&&manifest.decision==="not_evaluable"&&manifest.warning==="engineering_wiring_smoke_not_an_experiment_result"&&manifest.model===activeModel&&manifest.prompt_sha256===expectedPromptSha&&manifest.output_schema_sha256===expectedOutputSchemaSha&&manifest.protocol_fingerprint_sha256===expectedProtocolFingerprint(activeModel,activeSeeds[index])&&manifest.case_count===2&&manifest.arm_count===4&&manifest.seed_count===1&&manifest.request_count===8,`${run} manifest未满足冻结生成协议`);
  const blindBytes=await readFile(join(base,run,"blind/items.jsonl"));const recordsBytes=await readFile(join(base,run,"private/run-records.jsonl"));const pilotBytes=await readFile(join(base,run,"private/input-pilot.json"));
  const keys=JSON.parse(await readFile(join(base,run,"private/answer-key.json"),"utf8")) as KeyItem[];const blindItems=parseJsonLines<ActiveBlindItem>(blindBytes.toString("utf8"));const records=parseJsonLines<PrivateRunRecord>(recordsBytes.toString("utf8"));const pilot=JSON.parse(pilotBytes.toString("utf8")) as PilotPackage;
  exactKeys(pilot as unknown as Record<string,unknown>,["schema_version","bundle_id","protocol","samples","answer_key","blind_evaluation_items"],`${run} pilot`);exactKeys(pilot.protocol as unknown as Record<string,unknown>,["protocol_version","prompt_version","visual_items_per_visual_arm","speech_window_seconds","visual_budget_rule","runtime_pixel_and_token_audit_required"],`${run} pilot protocol`);
  assert(pilot.schema_version==="oracle-pilot-package-v1"&&pilot.bundle_id==="tbv2-ly-004-01-gold-dev-v2"&&pilot.protocol.protocol_version==="oracle-value-gate-v1"&&pilot.protocol.prompt_version===expectedPromptVersion&&pilot.protocol.visual_items_per_visual_arm===1&&pilot.protocol.speech_window_seconds===5&&pilot.protocol.visual_budget_rule==="one_preprocessed_canvas_per_visual_arm"&&pilot.protocol.runtime_pixel_and_token_audit_required===true,`${run} pilot未满足冻结协议`);
  assert(keys.length===8&&blindItems.length===8&&records.length===8&&pilot.samples.length===8&&pilot.answer_key.length===8&&pilot.blind_evaluation_items.length===8,`${run} source artifacts必须各含8项`);
  keys.forEach((item,itemIndex)=>exactKeys(item as unknown as Record<string,unknown>,["blind_id","paired_case_id","case_id","arm","seed","condition_sha256"],`${run} key ${itemIndex}`));blindItems.forEach((item,itemIndex)=>exactKeys(item as unknown as Record<string,unknown>,["blind_id","paired_case_id","seed_index","response","response_sha256"],`${run} blind ${itemIndex}`));records.forEach((item,itemIndex)=>{exactKeys(item as unknown as Record<string,unknown>,["run_id","blind_id","paired_case_id","case_id","arm","seed","condition_sha256","response_sha256","response","request_audit","canonical_visual"],`${run} record ${itemIndex}`);exactKeys(item.request_audit as unknown as Record<string,unknown>,["request_sha256","model","attempt_count","submitted_visuals","provider_response_received","stop_reason","usage","transport","temperature","max_output_tokens","seed","cache_retention","tools_policy"],`${run} audit ${itemIndex}`);});pilot.samples.forEach((item,itemIndex)=>exactKeys(item as unknown as Record<string,unknown>,["case_id","arm","evidence_mode","source_video_id","delta_id","time","speech_ids","transcript","evidence_text","image_assets","paired_context_sha256","condition_sha256"],`${run} pilot sample ${itemIndex}`));
  const keyByBlind=new Map(keys.map((item)=>[item.blind_id,item]));const blindById=new Map(blindItems.map((item)=>[item.blind_id,item]));const recordById=new Map(records.map((item)=>[item.blind_id,item]));assert(keyByBlind.size===8&&blindById.size===8&&recordById.size===8,`${run} source blind id必须唯一`);
  for(const expectedCase of expectedCases){const caseKeys=keys.filter((item)=>item.case_id===expectedCase.case_id);const pilotSamples=pilot.samples.filter((item)=>item.case_id===expectedCase.case_id);assert(caseKeys.length===4&&pilotSamples.length===4&&stableJson(caseKeys.map((item)=>item.arm).sort())===stableJson([...arms].sort())&&stableJson(pilotSamples.map((item)=>item.arm).sort())===stableJson([...arms].sort()),`${run}/${expectedCase.case_id}未精确覆盖四臂`);assert(caseKeys.every((item)=>item.seed===activeSeeds[index])&&pilotSamples.every((item)=>item.delta_id===expectedCase.delta_id),`${run}/${expectedCase.case_id} seed/delta漂移`);assert(pilotSamples.find((item)=>item.arm==="transcript_only")?.image_assets.length===0&&pilotSamples.filter((item)=>item.arm!=="transcript_only").every((item)=>item.image_assets.length===1&&/^[a-f0-9]{64}$/.test(item.image_assets[0].sha256)),`${run}/${expectedCase.case_id}视觉预算无效`);}
  for(const key of keys){const blind=blindById.get(key.blind_id),record=recordById.get(key.blind_id),pilotSample=pilot.samples.find((item)=>item.case_id===key.case_id&&item.arm===key.arm),pilotKey=pilot.answer_key.find((item)=>item.case_id===key.case_id&&item.arm===key.arm);assert(blind&&record&&pilotSample&&pilotKey,`${run} key/blind/record/pilot集合不闭合`);assert(blind.paired_case_id===key.paired_case_id&&blind.seed_index===0&&record.paired_case_id===key.paired_case_id&&record.case_id===key.case_id&&record.arm===key.arm&&record.seed===key.seed&&record.condition_sha256===key.condition_sha256&&pilotSample.condition_sha256===key.condition_sha256&&pilotKey.case_id===key.case_id&&pilotKey.arm===key.arm&&pilotKey.condition_sha256===key.condition_sha256,`${run} source identity漂移`);const audit=record.request_audit;assert(audit.model===activeModel&&audit.transport==="pi"&&audit.seed===activeSeeds[index]&&audit.attempt_count===1&&audit.provider_response_received===true&&audit.stop_reason==="stop"&&audit.temperature===0&&audit.max_output_tokens===2048&&audit.cache_retention==="none"&&audit.tools_policy==="none",`${run} request audit不符合冻结生成条件`);assert(stableJson(record.response)===stableJson(blind.response)&&record.response_sha256===blind.response_sha256&&sha(JSON.stringify(blind.response))===blind.response_sha256,`${run} response未由run record推出`);validateOracleGateResponse(blind.response,key.arm);
    const images:Array<{label:string;bytes:Buffer;mime_type:"image/jpeg";sha256:string}>=[];
    if(key.arm==="transcript_only")assert(record.canonical_visual===null&&audit.submitted_visuals.length===0,`${run} transcript视觉漂移`);else{assert(pilotSample.image_assets.length===1&&record.canonical_visual!==null&&audit.submitted_visuals.length===1,`${run} visual audit缺失`);const source=pilotSample.image_assets[0];const verified=await verifyImageEvidence({root:bundleRoot,assetUri:source.asset_uri,expectedSha256:source.sha256});const canvas=canonicalizeOracleGateCanvas(verified.bytes,key.arm);assert(record.canonical_visual.source_asset_uri===source.asset_uri&&record.canonical_visual.source_sha256===source.sha256&&record.canonical_visual.submitted_sha256===canvas.sha256&&record.canonical_visual.mime_type===canvas.mime_type&&record.canonical_visual.width===canvas.width&&record.canonical_visual.height===canvas.height&&record.canonical_visual.transform===canvas.transform&&audit.submitted_visuals[0].label==="visual-1"&&audit.submitted_visuals[0].sha256===canvas.sha256&&audit.submitted_visuals[0].mime_type===canvas.mime_type&&audit.submitted_visuals[0].byte_length===canvas.bytes.byteLength,`${run} visual未由pilot source确定性推出`);images.push({label:"visual-1",bytes:canvas.bytes,mime_type:canvas.mime_type,sha256:canvas.sha256});}
    const prompt=renderOracleGateUserPrompt(pilotSample,expectedPromptVersion);const requestSha=hashOracleGateRequest({model:activeModel,user:prompt,temperature:0,control:{transport:"pi",maxOutputTokens:2048,seed:activeSeeds[index],cacheRetention:"none"},images});assert(audit.request_sha256===requestSha,`${run} request hash未由冻结prompt/pilot/visual推出`);
  }
  sourceMap.push(...keys.map((item)=>({...item,development_seed_index:index,spec_sha256:specSha})));for(const item of blindItems){assert(!activeBlindById.has(item.blind_id),"active blind id重复");activeBlindById.set(item.blind_id,item);}expectedSourceRoots.push({development_seed_index:index,generation_spec_sha256:specSha,run_manifest_sha256:sha(manifestBytes),blind_items_sha256:sha(blindBytes),private_run_records_sha256:sha(recordsBytes),input_pilot_sha256:sha(pilotBytes),item_count:8});
}
...
const expectedMatrix={protocol_scope:expectedProtocolScope,protocol_document_sha256:sha(await readFile(resolve(root,expectedProtocolDocument))),case_count:2,arm_count:4,arms_sha256:sha(stableJson([...arms].sort())),development_seed_count:3,active_generation_seeds_sha256:sha(stableJson(activeSeeds)),request_count:24,model:activeModel,prompt_sha256:activePromptSha,output_schema_sha256:activeSchemaSha,source_roots:expectedSourceRoots,private_map_payload_sha256:privatePayloadSha,rater_item_roots:{R1:sha(await readFile(join(base,"blind-package/r1/items.jsonl"))),R2:sha(await readFile(join(base,"blind-package/r2/items.jsonl")))} };
...
let decision:"GO"|"FIX"|"PIVOT_STOP"|"FIX_STOP"="PIVOT_STOP";if(isFix){decision=oracle.evidence_f1>=0.9434523809523809&&oracle.edit_coverage===1&&oracle.operation_fidelity===1&&oracle.unsupported_claim_rate===0&&positiveCases===2&&positivePairs>=4&&oracle.hard_failures===0?"GO":"FIX_STOP";}else if(positiveCases===2&&positivePairs>=4&&oracle.edit_coverage>nonOracle.edit_coverage&&oracle.unsupported_claim_rate<=nonOracle.unsupported_claim_rate&&oracle.hard_failures===0)decision="GO";else if(oracle.evidence_f1>nonOracle.evidence_f1&&oracle.hard_failures===0)decision="FIX";
const result={schema_version:isFix?"ly004-development-fix-gate-result-v1":"ly004-development-value-gate-result-v2",status:"development_agent_rated_not_paper_result",decision,strongest_non_oracle_arm:nonOracle.arm,aggregation:"equal_rater_mean_then_global_item_mean; no adjudication",arm_summary:armSummary,paired_differences:paired,case_differences:caseDeltas,decision_evidence:{positive_cases:positiveCases,positive_case_seed_pairs:positivePairs,required_positive_pairs:4,oracle_edit_coverage_advantage:oracle.edit_coverage-nonOracle.edit_coverage,oracle_unsupported_claim_rate_difference:oracle.unsupported_claim_rate-nonOracle.unsupported_claim_rate,oracle_hard_failures:oracle.hard_failures,...(isFix?{fix_thresholds:{minimum_oracle_evidence_f1:0.9434523809523809,required_edit_coverage:1,required_operation_fidelity:1,required_unsupported_claim_rate:0}}:{})},rating_files:ratingFileHashes,warning:"two_independent_agent_ratings_for_development_direction_only_not_human_expert_or_paper_evidence"};
