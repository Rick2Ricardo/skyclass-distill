...
$("#openSettings").addEventListener("click",async()=>{
  const settings=await api("/api/settings"); $("#baseUrl").value=settings.llm_base_url||""; $("#modelName").value=settings.llm_model||""; $("#whisperModel").value=settings.whisper_model||"small"; $("#apiKey").placeholder=settings.llm_api_key_hint?`已保存 ${settings.llm_api_key_hint}，留空则保留`:"sk-…"; $("#settingsMessage").textContent=""; dialog.showModal();
});
function settingsPayload(){return {llm_base_url:$("#baseUrl").value.trim(),llm_api_key:$("#apiKey").value.trim(),llm_model:$("#modelName").value.trim(),whisper_model:$("#whisperModel").value};}
