...
from .models import (
    ...
    QARequest, QAResponse, QAVote, SearchRequest, SettingsUpdate, VideoDeleteRequest,
)
from .pi_agent import pi_agent_available
...
def create_api_router(manager: PipelineManager, settings_loader: SettingsLoader) -> APIRouter:
    ...
    def health():
        ...
        return {
            ...
            "pi_agent_available": pi_agent_available(),
        }
    ...
    def list_project_qa(project_id: str):
        try:
            manager.library.get_project(project_id)
        except KeyError as exc:
            raise HTTPException(404, "项目不存在") from exc
        return [
            {
                ...
                "student_response": job.qa_student_response,
                "created_at": job.created_at,
                "updated_at": job.updated_at,
                "result": job.artifacts.get("qa") if job.status == "completed" else None,
                "error": job.error,
            }
            for job in manager.store.list()
            if job.project_id == project_id and job.kind == "qa"
        ]

    @router.post("/projects/{project_id}/qa")
    def create_project_qa(project_id: str, payload: QARequest):
        try:
            job = manager.create_qa(project_id, payload)
        except KeyError as exc:
            raise HTTPException(404, "项目不存在") from exc
        except ValueError as exc:
            raise HTTPException(400, str(exc)) from exc
    ...
    @router.post("/jobs/{job_id}/qa/reveal")
    def reveal_qa(job_id: str, payload: QAVote):
        try:
            return manager.reveal_qa(job_id, payload.choice)
        except KeyError as exc:
            raise HTTPException(404, "QA 任务不存在") from exc
        except ValueError as exc:
            raise HTTPException(400, str(exc)) from exc

    @router.post("/jobs/{job_id}/qa/respond")
    def respond_to_qa(job_id: str, payload: QAResponse):
        try:
            job = manager.create_qa_followup(job_id, payload.student_response)
        except KeyError as exc:
            raise HTTPException(404, "QA 任务不存在") from exc
        except ValueError as exc:
            raise HTTPException(400, str(exc)) from exc
